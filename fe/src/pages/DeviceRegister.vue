  ...
  if (isEditMode.value) {
    try {
      ...
      if (data.status === "success") {
        ...
        avatarPreview.value =
          d.image_url || (d.avatar ? `/api/web/avatar/${d.avatar}` : "");
      } else {
...
const submit = async () => {
  ...
  try {
    ...
    if (!response.ok || payload.error) {
      ...
      serverError.value =
        payload.error ||
        (isEditMode.value ? "Cap nhat that bai." : "Dang ky that bai.");
  ...
  <div class="page">
    <div class="card">
      <header class="card__header">
        <div>
          <p class="eyebrow">
            {{ isEditMode ? "Update Device" : "Device registration" }}
          </p>
      ...
      <div v-else class="complete">
        <div class="complete__box">
          <h2>
            {{ isEditMode ? "Cap nhat thanh cong" : "Dang ky thanh cong" }}
          </h2>
          <p>
            {{
              isEditMode
                ? "Thong tin thiet bi da duoc cap nhat."
                : "Ban da dang ky thiet bi thanh cong."
            }}
          </p>
